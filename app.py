...
import os
# modules 資料夾名稱，引用這個資料的 faq & menu資源
...
def handle_message(event):
    with ApiClient(configuration) as api_client:
        ...
        bot_msg = TextMessage(text=f"親愛的使用者，您剛才說的是: {user_msg}")

        # 當使用者輸入的文字作為faq的key時，就將value作為bot_msg 
        if user_msg in faq:
            bot_msg = faq[user_msg]
        elif user_msg.lower() in["menu","選單","主選單","主頁","首頁","我有問題","想問","我想問"]:
            bot_msg = menu
            ...
            completion = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "你是一個親切的教授，甚麼問題都可以回答"},
                    {
                        "role": "user", 
                        "content": user_msg
                    }
        ...
        line_bot_api.reply_message_with_http_info(
            ReplyMessageRequest(
                ...
                messages=[
                    # 要回應的內容放在這個串列中
                    TextMessage(text="逆好，請輸入menu觀看主選單"),
                    bot_msg
                ]
            )
        )
